...
function PANEL:CreateOptions(tbl)
    local button1 = vgui.Create("jlib_button", self)
    button1:SetText(tbl["button1"].text)
    button1:SetSize(self:GetWide() / 2 - 10, 30)
    button1:SetPos(jlib.Scaling.ScaleW(5), jlib.Scaling.ScaleH(30))
    button1:Inverse()
    button1.DoClick = function()
        tbl["button1"].func()
    end

    local button2 = vgui.Create("jlib_button", self)
    button2:SetText(tbl["button2"].text)
    button2:SetSize(self:GetWide() / 2 - 10, 30)
    button2:SetPos(self:GetWide() / 2 + 5, jlib.Scaling.ScaleH(30))
    button2:Inverse()
...
vgui.Register("jlib_popup", PANEL, "jlib_frame")