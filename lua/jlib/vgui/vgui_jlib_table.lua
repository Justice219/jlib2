    ...
    for k, v in pairs(self.Items) do
        if v.Type == "player" then
            ...
            container:ClearPaint()
                :Background(jlib.Colors.Secondary, 0)
            ...
            if self.PlayerShowSteamID then
                local steamid = container:Add("jlib_button")
                steamid:Dock(RIGHT)
        ...
        elseif v.Type == "item" then
            ...
            value:SetFont("jlib.font.button")
            value:Dock(RIGHT)
            value:DockMargin(15,0,15,0)
            value:SetText(v.Data.value)
            value:SetTextColor(jlib.Colors.White)
...
vgui.Register("jlib_table", PANEL, "DScrollPanel")